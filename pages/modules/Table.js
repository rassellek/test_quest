...
export default function MyTable(props) {
  ...
  const onChangeData = (resp) =>
    setData(
      formatting(resp).map((it) => ({
        ...it,
        key: it.id ? it.id : 0,
      }))
    );
    ...
    if (typeof columns == "object")
      for (let column of columns) {
        data.push({
          ...
          title:
            column.title +
            (column.hasOwnProperty("suffix") ? ", " + column.suffix : ""),
          ...
          sorter: column.hasOwnProperty("sorter")
            ? (a, b) => a[column.key] - b[column.key]
            : false,
          sortOrder: !column.hasOwnProperty("sorter")
            ? null
            : sortedInfo.columnKey === column.key
            ? sortedInfo.order
            : null,
        ...
        let render = column.hasOwnProperty("render")
          ? column.render
          : (text) => (String(text) ? text : "");
        if (column.isCount && column.key != "id")
          render = (text) =>
            !Number.isInteger(text) ? text : text.toLocaleString();
        if (column.type == "trend")
          render = (text) => {
            if (text > 0) {
              return <Text type="success">+{text}%</Text>;
            } else if (text < 0) return <Text type="danger">-{text}%</Text>;
            else return <Text>{text ? text + " %" : ""}</Text>;
          };
        else if (column.type == "link" && column.link)
          render = (text) => (
            <Link href={text ? `/wb/${column.link}/` + text.id : ""}>
              {typeof text == "object" && text.hasOwnProperty("name")
                ? text.name
                : ""}
            </Link>
          );
          ...
          const popoverContent = text.src && (
            <Space>
              ...
              <Descriptions
                size="small"
                column={1}
                style={{ maxWidth: 400, marginLeft: 16 }}
              >
                {record &&
                  data.map((item, index) => {
                    if (index != 0)
                      return (
                        item && (
                          <Descriptions.Item
                            label={item && columns[index].title}
                          >
            ...
            <div style={{ marginTop: -8 }}>
              {text.src && (
                ...
                >
                  <Image
                    src={text.src("tm")}
                    preview={{ src: text.src("big"), mask: <EyeOutlined /> }}
                    height={40}
                  />
                </Popover>
              )}
              <Tooltip title={text.name}>
                <Link
                  href={text ? `/wb/${tab ? tab : type}/` + text.id : ""}
                  style={{ marginLeft: 10 }}
                >
                  {text.name}
                </Link>
  ...
  function addFavorites() {
    axiosAPI
      .post(`/api/fav/${!isFavorites ? "append" : "delete"}`, {
        type: type,
        id: selectedRowKeys,
      })
      .then((response) => {
        if (!isFavorites) message.success("Добавлено в избранное!");
        else {
...
//         if (days != 30) axiosUrl.searchParams.set("days", days);
//       }
...
//         .get(axiosUrl)
//         .then((response) => {
//           if (pagg) setData(data.concat(formatting(response)));
//           else onChangeData(response);
//         })
//         .finally(() => {
...
//   }, [url, dates]);
  ...
  useEffect(() => {
    if (filters === false) {
      ...
      if (!filters[0].hasOwnProperty("false")) setFilteredInfo(filters[0]);
  ...
  const onChange = (pagination, filters, sorter) => {
    ...
    for (let filter in filters)
      if (filters[filter]) filt = { filter: filters[filter] };
    ...
    if (filt === false) {
      ...
      } else setFilters(false);
    } else setFilters([filters, sorter]);
  };
